...
class PyCFDGovernanceAdapter:
    ...
    Provides validation bridging, candidate record construction, session event
    emission, and runtime evidence persistence to SessionStore, AuditLog, and
    ProvGraph backends.
    """
    ...
    def __init__(
        ...
        *,
        ...
        actor: str = "pycfd_governance_adapter",
    ) -> None:
    ...
    def build_core_validation_report(
        ...
        """Convert PyCFD validation + policy reports to MHE core ValidationReport."""
    ...
    def build_candidate_record(
        ...
        *,
        ...
        """Build a CandidateRecord from evidence bundle and policy evaluation."""
            ...
            dict.fromkeys(
                [
                    ...
                    *validation.evidence_refs,
                    f"pycfd://run/{validation.task_id}/{validation.run_id}",
                    f"pycfd://artifact/{validation.artifact_ref}",
                ]
            )
        )
    ...
    def _policy_gate_issues(
        ...
        """Extract ValidationIssue list from non-passing policy gates."""
        ...
        for gate in policy.gates:
            ...
            issues.append(
                ValidationIssue(
                    code=f"pycfd_gate_{gate.get('gate', 'unknown')}",
                    message=gate.get("reason", "Gate did not pass."),
                    subject=validation.task_id,
                    category=ValidationIssueCategory.PROMOTION_BLOCKER,
                    blocks_promotion=result == "reject",
                )
            )
        return issues
    ...
    def _resolve_graph_version(
        ...
        """Resolve graph version from bundle metadata or plan."""
    ...
    def _resolve_candidate_id(
        ...
        """Resolve candidate ID from bundle metadata, plan, or artifact."""
        metadata = bundle.metadata
        plan = bundle.plan
        raw_candidate_id = (
            metadata.get("candidate_id")
            or (plan.promotion_metadata.get("candidate_id") if plan else None)
            or (f"pycfd-candidate-v{snapshot.graph_version}" if snapshot.graph_version else None)
            or validation.artifact_ref
        )
        return str(raw_candidate_id)

    def _require_validation(self, bundle: PyCFDEvidenceBundle) -> PyCFDValidationReport:
        """Require a validation report from the bundle, raising if missing."""
        if bundle.validation is None:
            raise ValueError("PyCFD evidence bundle requires an attached validation report")
