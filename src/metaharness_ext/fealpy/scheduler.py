...
class FealpySchedulerAdapter:
    ...
    def _backend(self, plan: FealpyRunPlan) -> FealpySlurmBackend:
        backend_name = str(plan.graph_metadata.get("target_backend") or "slurm")
        if backend_name == "k8s":
            if self._k8s is None:
                raise ValueError("K8s backend is not implemented for fealpy")
            return self._k8s  # type: ignore[return-value]
        if backend_name not in {"slurm"}:
            raise ValueError(f"Unsupported fealpy scheduler backend: {backend_name}")
        return self._slurm

    def _backend_for_job(self, job_id: str) -> FealpySlurmBackend:
