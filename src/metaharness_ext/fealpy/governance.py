...
class FealpyGovernanceAdapter:
    def __init__(
        self,
        *,
        session_id: str | None = None,
        actor: str = "fealpy_governance_adapter",
    ) -> None:
    ...
    def build_core_validation_report(
        self,
        validation: FealpyValidationReport,
        policy: FealpyPolicyReport,
    ) -> ValidationReport:
            ...
            dict.fromkeys(
                [
                    ...
                    *validation.evidence_refs,
                    f"fealpy://run/{validation.task_id}/{validation.run_id}",
                    f"fealpy://artifact/{validation.artifact_ref}",
                ]
            )
        )
    ...
    def _resolve_candidate_id(
        ...
        raw_candidate_id = (
            ...
            or (f"fealpy-candidate-v{snapshot.graph_version}" if snapshot.graph_version else None)
            or validation.artifact_ref
        )
        return str(raw_candidate_id)

    def _require_validation(self, bundle: FealpyEvidenceBundle) -> FealpyValidationReport:
        if bundle.validation is None:
            raise ValueError("Fealpy evidence bundle requires an attached validation report")
