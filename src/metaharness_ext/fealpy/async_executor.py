...
class FealpyAsyncExecutor:
    def __init__(self, executor: FealpyExecutorComponent | None = None) -> None:
        self._executor = executor or FealpyExecutorComponent()
        self._handles: dict[str, JobHandle] = {}
        self._results: dict[str, FealpyRunArtifact] = {}

    async def submit(self, plan: FealpyRunPlan) -> JobHandle:
        handle = JobHandle(
            job_id=plan.run_id, backend="fealpy-local", status=ExecutionStatus.RUNNING
        )
    ...
    async def await_result(self, job_id: str, timeout: float | None = None) -> FealpyRunArtifact:
        if job_id not in self._results:
            raise KeyError(f"Fealpy job has no result: {job_id}")
    ...
    def _require_handle(self, job_id: str) -> JobHandle:
        try:
            ...
            raise KeyError(f"Unknown Fealpy job: {job_id}") from error


def _artifact_status(artifact: FealpyRunArtifact) -> ExecutionStatus:
