...
- 保持 manifest `policy.credentials` / `policy.sandbox` 的实现、tests、docs 一致
...
- 收紧仍偏宽松的 evidence / prerequisite 语义并补齐边界回归
...
3. NSCF 目前接受 `running_nscf.log` 或 `running_scf.log` 作为成功证据之一，这一规则较宽松，后续可以再收紧
4. 可补更多边界测试：
