...
如果先把这些点测稳，后续再进入 smoke policy 与 richer diagnostics interpretation 时会轻松很多。
...
### 第四层：minimal execution demo / smoke / e2e tests

当前应覆盖两类基础 e2e：

- minimal execution demo（graph wiring + schema / validate-only / real-run）
- real-run foundation / smoke-policy-adjacent coverage
...
- `test_metaharness_jedi_minimal_demo.py`
- `test_metaharness_jedi_smoke.py`

当前实现中还已经存在并应被视为事实基线的测试面：

- `test_metaharness_jedi_diagnostics.py`
- `test_metaharness_jedi_validator_diagnostics.py`
- `test_metaharness_jedi_smoke_policy.py`
- `test_metaharness_jedi_study.py`
- `test_metaharness_jedi_variational_e2e.py`
- `test_metaharness_jedi_letkf_e2e.py`

这组测试的价值在于：它们共同覆盖 package、contract、环境、命令构造、minimal runtime evidence，以及 smoke policy / diagnostics / study 的当前实现事实。
...
- Phase 0：package / contract / env / mode-aware execution / evidence-first validation
- Phase 1：smoke policy / artifact interpretation / scientific acceptance split
- Phase 2：variational baseline + minimum scientific evidence
- Phase 3：ensemble baseline
- Phase 4+：更强的 diagnostics summary / scored evidence / governance integration

hot-swap、session-event 一类能力当前更适合被当作 governance integration hook 来测试，而不是把它们写成 JEDI 自主执行主线。
