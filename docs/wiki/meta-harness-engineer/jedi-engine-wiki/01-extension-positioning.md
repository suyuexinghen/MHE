...
```text
spec -> environment probe -> controlled YAML -> validate-only -> validation report
```

后续再扩展到：

```text
spec -> environment probe -> preprocess -> real run -> diagnostics -> validation report -> study
```
...
- `schema` / `validate-only` / `real-run` 三类模式需要分层建模
