...
> 状态：全部 Phase 0–5 + Gateway + Quafu 已实现 | 远期扩展待规划 | 以 `qcompute-engine-wiki` (01–08) 和 `06-qcompute-extension-blueprint.md` 为准的执行路线图
...
Phase 0: Scaffold + Contracts + Mock Backend             [当前阶段]
  -> Phase 1: Qiskit Aer Simulation Baseline             [完整模拟器闭环]
    -> Phase 2: Quafu Environment Probe + Real Backend    [真机接入]
      -> Phase 3: Error Mitigation (自实现 ZNE/REM)       [噪声对抗]
...
1. `backends/quafu.py`：QuafuBackendAdapter（quarkstudio SDK）
...
5. 测试：
   - 环境探测单元测试（mock quarkstudio）
   - 异步轮询逻辑测试
   - 校准时效性测试
   - 配额感知调度测试
   - 真机冒烟测试（需 `Qcompute_Token`，标记 `@pytest.mark.quafu`）

### 6.4.3 前提条件

- quarkstudio SDK 已安装（`pip install quarkstudio`）
- 有效的 `Qcompute_Token` 环境变量（真机测试）
...
## 6.5 Phase 3：Error Mitigation (自实现 ZNE/REM)

### 6.5.1 目标

通过自实现错误缓解能力（ZNE/REM，纯 NumPy），提升噪声环境下的结果质量。注意：`mitiq` 与 Python 3.13 不兼容，因此 ZNE（Richardson 外推 + 幺正折叠）和 REM（逆混淆矩阵）采用纯 NumPy 自实现。

### 6.5.2 交付内容

1. `mitigation.py`：自实现 ZNE/REM
   - ZNE：Richardson 外推（Vandermonde 求解），scale factors [1, 3, 5]
   - REM：逆混淆矩阵校正
   - 幺正折叠（unitary folding）用于噪声放大
   - `_apply_error_mitigation()` 方法
   - 错误缓解开销通过 `QComputeExecutionPolicy` 记录
   - 噪声影响评分高时，若 core tri-state policy 已落地，可产出 `DEFER` 决策
2. `mitigation_pennylane.py`：PennyLane 集成错误缓解
3. `config_compiler.py` 增强：
...
| Layer 3 | 真机冒烟测试（Quafu） | quarkstudio + token | `@pytest.mark.quafu` |
