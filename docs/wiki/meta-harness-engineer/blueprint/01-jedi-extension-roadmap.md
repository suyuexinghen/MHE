...
把 Phase 0 的基础执行面推进到“基于当前 real-run 能力选择并稳固一个 toy smoke policy”，验证更高层 wrapper 主链：
...
5. 扩 `JediValidationReport` / validator，补 `blocking_reasons`、`policy_decision`、`prerequisite_evidence` 与 refs 等最小治理语义
6. 在此基础上补独立 evidence / policy layer，使 validator/report 成为 candidate review、runtime authority、audit 与 provenance flow 的稳定 handoff 输入
