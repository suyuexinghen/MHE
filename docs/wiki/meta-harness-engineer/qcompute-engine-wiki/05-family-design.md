...
1. **Propose**：`BrainProvider.propose(optimizer, observations)` 返回 `list[MutationProposal]`。
   每条 MutationProposal 的 payload 携带 `QComputeExperimentSpec` 实例，
   表达"尝试哪个 ansatz × backend × error_mitigation 组合"的意图。
   Agent 内部如何生成该 payload（如 QAgent 的 Planner-Coder-Reviewer 三 agent 模式）
   是 `propose()` 的实现细节，MHE 框架不感知。
2. **Execute**：Study 组件从 MutationProposal 中提取 ExperimentSpec，
   通过 Gateway 编排每个 trial 的执行
3. **Evaluate**：`BrainProvider.evaluate(optimizer, proposal, observations)` 返回
   `ProposalEvaluation`，内含保真度、能量误差等量子指标。
   Study 组件将其映射到 `QComputeStudyTrial.trajectory_score`
4. **Refine**：基于 Pareto 前沿，下一轮 `propose()` 调用接收更新后的 observations，
   Agent 提议新的候选区域
