...
## 1.7 更长期的 CMA-inspired 基础设施方向

在当前 MHE 与既有强化路线图之外，还可以把一部分能力理解为更长期的基础设施演进方向，而不是近期 patch list：

- **追加型会话基底**：以 `SessionStore` / `SessionEvent` 统一承载状态恢复、审计与观测事实，使会话事件成为更稳定的系统事实源
- **运行时去状态化**：逐步减少 `HarnessRuntime` 对进程内活动对象的依赖，朝 `wake(session_id)` 驱动的恢复式编排靠拢
- **更强执行边界**：把惰性沙箱从验证手段进一步推向执行面边界，使高风险工具与普通编排路径更清晰解耦
- **模型抽象上移**：以 `BrainProvider` 明确推理提供者边界，使 Harness 更像模型无关的控制面，而非隐式绑定某一种 LLM 接口

这类方向的作用，是帮助读者理解 MHE 可能如何从“可重构的工程运行时”继续演进到“更偏控制面的 Agent 基础设施”；它补充当前路线图，但不替代当前路线图。
