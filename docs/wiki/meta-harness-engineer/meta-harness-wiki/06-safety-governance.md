...
## 6.1.1 protected boundary 与治理要求

除了沙箱与 policy veto，本章还约束受保护组件边界：
- protected components 是治理链中的高优先级位点，不能被普通优化动作直接覆盖；
- 对 protected components 的重接线、模板替换、热替换都必须经过统一治理与审计；
- extension validator 常常也是 protected boundary，因为它承担 evidence-first promotion gate 的角色。

这与 `03-core-components.md` 中的 protected 摘要互补；更正式的规则来源仍是 `docs/adr/ADR-004-protected-components.md`。
...
在现有治理补强之外，CMA-inspired 方向更强调把安全做成**架构边界**而不只是规则集合。

- **惰性沙箱作为执行边界**：当前 MHE 已有 sandbox ladder 与门控抽象；更长期可把惰性进程/容器沙箱提升为默认高风险执行边界，而不是仅作为验证阶段附件
- **`Credential Vault` 替代最小身份边界**：当前 `InMemoryIdentityBoundary` 更接近最小证明与隔离骨架；长期方向是让凭证引用、注入、撤销与审计走独立保险库路径
- **治理事件化**：把 allow / deny / escalate / rollback 等决策进一步沉淀为结构化事件，使治理与审计共享同一追加型事实源

这部分属于长期基础设施演进，重点是把“安全规则有效”进一步推进到“越权路径在架构上更难出现”。
