...
| PyCFD PDE | PyCFD has real solver execution evidence for all five current cases and cross-extension comparison docs. | PyCFD currently has the strongest real execution evidence among the listed PDE extensions, as a timestamped process-state observation. | Does not prove PyCFD is numerically superior to Fealpy, Nektar, Octave, QCompute, or direct Claude Code. | Expose direct proposal source, run real direct/agent comparisons without fallback ambiguity, and collect repeated pass/repair rates. |
| Fealpy PDE | Current comparison docs identify Fealpy as dry-run only in cross-extension evidence state. | Fealpy has benchmark plumbing and multi-backend design surface, but current comparison evidence is not real execution evidence. | Does not validate numpy/pytorch/jax backend claims as real execution evidence yet. | Execute real backend runs and compare evidence with PyCFD/Nektar only by domain-specific metrics. |
...
PyCFD currently has the strongest timestamped real solver evidence among listed PDE extensions: all five current PyCFD cases compile, execute, and produce residual/timing/cell-count metrics through the MHE benchmark pipeline. This is an evidence-state comparison, not a claim that PyCFD is scientifically or numerically better than other solvers.

Current claim boundary:

> PyCFD evidence supports real execution maturity for the current 2D Euler catalog. It does not prove cross-solver numerical superiority or direct-Claude code-generation success when fallback compiler paths are used.

### Cross-extension PDE comparison

The PDE cross-extension comparison currently says PyCFD has real execution evidence, while Fealpy and Nektar entries in that snapshot were dry-run-only. Later Nektar real-run evidence improves the Nektar state for selected cases, so this report should be updated whenever a suite’s real-run status changes.
