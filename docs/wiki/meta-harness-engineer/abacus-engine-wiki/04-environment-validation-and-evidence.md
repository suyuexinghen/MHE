...
它的价值不只是 preflight，而是把运行前提显式变成后续治理输入。

---

## 4.3 canonical failure taxonomy
...
它们不只是 extension-local 状态，也是更上层治理分类入口。
...
这说明 ABACUS validator 已经不是普通 post-run helper，而是当前 extension-local 的 protected governance boundary。
...
这样 extension 即使仍在开发中，failure taxonomy 与 evidence semantics 也能保持稳定。
