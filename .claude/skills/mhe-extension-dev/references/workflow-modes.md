...
Start here:
1. Read MHE framework docs, target software docs, target source README/tutorials/examples, and existing extension patterns.
2. Resolve source-truth conflicts before design. If docs and source describe different software families, anchor implementation and claims to the target source tree; cite the mismatch in docs as a truth boundary.
3. Identify the numbered blueprint slot for the extension.
4. Create the extension design wiki from `wiki-outline-template.md`.
5. Create the numbered blueprint, roadmap, and implementation-plan docs.
6. Then move into the smallest implementation slice.

Minimum runnable slice:
- Define typed contracts, capabilities, slots, and component declarations.
- Add package manifests and example manifests that match `declare_interface` output.
- Implement a gateway/intake, environment probe, compiler/config builder, executor, validator, and evidence/policy path only when each can be truthfully tested or bounded.
- Cover runtime execution with mocked binaries/subprocesses unless the user explicitly requests opt-in external solver tests.
- Document supported, mocked, unverified, external-prerequisite, and non-goal boundaries.
...
- Check manifest / `declare_interface` / contract drift, including sandbox tier versus actual file/process behavior.
- Check artifact identity provenance: evidence bundles should preserve real plan, artifact, validation, and run identifiers without substituting derived IDs.
- Check executor output discovery for configured workspace output directories, not only default paths.
- Check docs / handoff / roadmap truthfulness, including supported, mocked, unverified, external-prerequisite, and non-goal boundaries.
- Require an independent reviewer/verifier verdict of `complete` or `needs changes` before closing implementation slices that affect docs, roadmap, or claims.
